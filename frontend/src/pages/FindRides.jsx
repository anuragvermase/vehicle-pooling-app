...
const OfferRide = ({ user, onLogout }) => {
  ...
  const geocodeAddress = async (address) => {
    ...
    try {
      // Prefer client-side Google Geocoder if available
  ...
  const getMockCoordinates = (address) => {
    ...
    const k = Object.keys(mockLocations).find((city) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build payload aligned with backend (GeoJSON normalization happens server-side)
      const ridePayload = {
        startLocation: {
